/**
 * Centralized language and locale definitions for the backend
 * This ensures consistent language designations across the entire app
 * Must match frontend/src/utils/languages.ts
 */
...
/**
 * Map locale codes to language names (for backward compatibility with orchestrator)
 */
export const LOCALE_TO_LANGUAGE: Record<string, string> = {
  ...Object.fromEntries(SUPPORTED_LANGUAGES.map((lang) => [lang.code, lang.name])),
  ...Object.fromEntries(Object.entries(LOCALE_VARIANTS).map(([code, lang]) => [code, lang.name])),
};
